...
def sign_up2():
    ...
    cursor.execute(c)
    cursor.execute("use game")
    cursor.execute("create table if not exists score(name char(20),password char(20))")
    d='''select * from score'''
    cursor.execute(d)
    p=cursor.fetchall()
    for i in p:#anirudha read
        if (i[0]==text):
            largeText1 = pygame.font.Font('freesansbold.ttf',40)
            text_surface=largeText1.render("USERNAME ALREADY EXIST!!",True,red)
            win.blit(text_surface,(420,450))
            pygame.display.update()
            time.sleep(2)
            Sign_up()
    else:
        cursor.execute("insert into score values('{}','{}')".format(text,password))#to here
    m.commit()
    main()
def game_intro():
    intro = True
